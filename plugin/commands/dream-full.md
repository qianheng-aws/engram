...
```bash
python3 /workplace/qianheng/ooda-memory/dream_cli.py integrate --vault ~/.meshclaw/vault
```
Review candidates and merge if needed.

**3. Prune** — Check for decaying entities:
```bash
python3 /workplace/qianheng/ooda-memory/dream_cli.py prune --vault ~/.meshclaw/vault
```
Review fading/archivable entities.

**4. Abstract** — Discover behavioral patterns:
```bash
python3 /workplace/qianheng/ooda-memory/dream_cli.py abstract --vault ~/.meshclaw/vault
```
Analyze the returned daily notes for recurring behaviors, decision preferences, and problem patterns. Output JSON and save:
```bash
echo '<json>' | python3 /workplace/qianheng/ooda-memory/dream_cli.py save-pattern --vault ~/.meshclaw/vault --stdin
```
