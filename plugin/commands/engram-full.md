...
```bash
python3 /workplace/qianheng/ooda-memory/engram_cli.py integrate --vault ~/.engram/vault
```
Review candidates and merge if needed.

**3. Prune** — Check for decaying entities:
```bash
python3 /workplace/qianheng/ooda-memory/engram_cli.py prune --vault ~/.engram/vault
```
...
```bash
echo '<json>' | python3 /workplace/qianheng/ooda-memory/engram_cli.py save-pattern --vault ~/.engram/vault --stdin
```
