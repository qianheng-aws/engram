...
```bash
python3 /workplace/qianheng/ooda-memory/dream_cli.py status --vault ~/.engram/vault
```
...
```bash
echo '<json>' | python3 /workplace/qianheng/ooda-memory/dream_cli.py replay --vault ~/.engram/vault --stdin
```
